...
public class CreateCommand extends ProxyCommand {
    ...
    public CreateCommand(@NotNull final ProxyPlugin plugin) {
        super(plugin);
    }
    ...
    public void execute(@NotNull final CommandSender commandSender, @NotNull final String[] arguments) {
        if (arguments.length != 4) {
            commandSender.sendMessage(new TextComponent(super.getConfiguration().getString("messages.commands.permission.group.create.usage")
        ...
        if (groupProvider.getGroup(arguments[2].toLowerCase()).isPresent()) {
            commandSender.sendMessage(new TextComponent(super.getConfiguration().getString("messages.commands.permission.group.create.group-already-exists")
                    .replaceAll("%prefix%", super.getConfiguration().getPrefix())));
            return;
        }

        if (!Numbers.isInteger(arguments[3])) {
            commandSender.sendMessage(new TextComponent(super.getConfiguration().getString("messages.general.invalid-identification")
                    .replaceAll("%prefix%", super.getConfiguration().getPrefix())));
            return;
        }
        ...
        if (groupProvider.getGroup(groupIdentification).isPresent()) {
            commandSender.sendMessage(new TextComponent(super.getConfiguration().getString("messages.commands.permission.group.create.group-already-exists-id")
                    .replaceAll("%prefix%", super.getConfiguration().getPrefix())));
            return;
        }
        ...
        commandSender.sendMessage(new TextComponent(super.getConfiguration().getString("messages.commands.permission.group.create.successfully-created")
                .replaceAll("%prefix%", super.getConfiguration().getPrefix())
